...
class SoftDeleteBehavior extends Behavior
{
    ...
    /**
     * @var array values of the owner attributes, which should be applied on restoration from "deleted" state,
     ...
     * @see safeDelete()
     */
    public string $deleteFallbackException = IntegrityException::class;
     ...
     */
    public function setReplaceRegularDelete(bool $replaceRegularDelete): void
    {
     ...
     * @since 1.0.4
     */
    public function setUseRestoreAttributeValuesAsDefaults(bool $useRestoreAttributeValuesAsDefaults): void
    {
     ...
     *
     * @return false|int the number of rows marked as deleted, or false if the soft deletion is unsuccessful for some reason.
    ...
    {
        if (method_exists($this->owner, 'beforeSoftDelete') && !$this->owner->beforeSoftDelete()) {
            return false;
        }
     ...
     *
     * @return false|int the number of restored rows, or false if the restoration is unsuccessful for some reason
     */
    ...
    {
        if (method_exists($this->owner, 'beforeRestore') && !$this->owner->beforeRestore()) {
            return false;
        }
     ...
     * @return false|int the number of rows marked as deleted, or false if the soft deletion is unsuccessful for some reason
     */
    protected function softDeleteInternal(): false|int
    {
        $result = false;
    ...
    {
        if (!empty($this->restoreAttributeValues)) {
